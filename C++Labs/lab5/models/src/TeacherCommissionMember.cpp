...
istream& operator>>(istream& is, TeacherCommissionMember& tcm) {
    ...
    while (!success) {
        try {
            safeInputText(is, tcm.commissionName, "Enter commission name: ");
            success = true;
    ...
    while (!success) {
        try {
            safeInputDate(is, tcm.appointmentDate, "DD/MM/YYYY", "Enter appointment date: ");
            success = true;
        } catch (const InputException& e) {
            cout << "Error: " << e.what() << endl;
        }
    ...
    while (!success) {
        try {
            safeInputText(is, tcm.certificateNumber, "Enter certificate number: ");
            success = true;
        } catch (const InputException& e) {
            cout << "Error: " << e.what() << endl;
        }
    ...
    while (!success) {
        try {
            safeInputInt(is, tcm.autobiographyCount, 0, AUTOBIOGRAPHY_SIZE, "Enter number of autobiography entries (0-" + to_string(AUTOBIOGRAPHY_SIZE) + "): ");
            success = true;
        } catch (const InputException& e) {
            cout << "Error: " << e.what() << endl;
        }
    ...
    for (int i = 0; i < tcm.autobiographyCount; i++) {
        ...
        while (!success) {
            try {
                safeInputText(is, tcm.autobiography[i], "Enter autobiography entry " + to_string(i + 1) + ": ");
                success = true;
            } catch (const InputException& e) {
                cout << "Error: " << e.what() << endl;
            }
    ...
    while (!success) {
        try {
            safeInputInt(is, tcm.commissionWorksCount, 0, COMMISSION_WORKS_SIZE, "Enter number of commission works (0-" + to_string(COMMISSION_WORKS_SIZE) + "): ");
            success = true;
        } catch (const InputException& e) {
            cout << "Error: " << e.what() << endl;
        }
    ...
    for (int i = 0; i < tcm.commissionWorksCount; i++) {
        ...
        while (!success) {
            try {
                safeInputText(is, tcm.commissionWorks[i], "Enter commission work " + to_string(i + 1) + ": ");
                success = true;
            } catch (const InputException& e) {
                cout << "Error: " << e.what() << endl;
            }
